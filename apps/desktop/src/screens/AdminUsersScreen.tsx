            ...
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead className="bg-gray-50 dark:bg-gray-900">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">注册</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">会员</th>
                ...
                {users.length === 0 ? (
                  ...
                  users.map((user) => (
                    <tr key={user.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="text-sm font-medium text-gray-900 dark:text-white">
                              ...
                              {user.role === 'admin' && (
                                <span className="ml-1.5 px-1 py-0.5 rounded text-xs bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400">管理员</span>
                              )}
                            </div>
                      ...
                      <td className="px-4 py-3">
                        <button onClick={() => openVipModal(user)} className="hover:underline">
                          {getVipBadge(user.vipLevel) || <span className="px-1.5 py-0.5 rounded text-xs bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-400">免费</span>}
                        </button>
                      </td>
                      <td className="px-4 py-3">
                        <button
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between px-4 py-3 border-t border-gray-200 dark:border-gray-700">
                ...
                <div className="flex gap-2">
                  <Button size="sm" variant="ghost" onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1}>上一页</Button>
                  <Button size="sm" variant="ghost" onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages}>下一页</Button>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-lg w-full mx-4 max-h-[80vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-xl font-bold text-gray-900 dark:text-white">{selectedUser.displayName || selectedUser.username}</h3>
                  <p className="text-gray-500">@{selectedUser.username}</p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div><label className="text-xs text-gray-500">邮箱</label><p className="text-sm text-gray-900 dark:text-white">{selectedUser.email}</p></div>
                <div><label className="text-xs text-gray-500">注册时间</label><p className="text-sm text-gray-900 dark:text-white">{formatDate(selectedUser.createdAt)}</p></div>
                <div><label className="text-xs text-gray-500">最后登录</label><p className="text-sm text-gray-900 dark:text-white">{formatDate(selectedUser.lastLoginAt)}</p></div>
                <div><label className="text-xs text-gray-500">收藏/阅读/书签</label><p className="text-sm text-gray-900 dark:text-white">{selectedUser._count?.collections || 0} / {selectedUser._count?.readingProgress || 0} / {selectedUser._count?.bookmarks || 0}</p></div>
              </div>
            </div>
      ...
      {showVipModal && selectedUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-md w-full mx-4">
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">会员等级</label>
                <select
                  value={vipEdit.level}
            ...
            <div className="p-6 border-t border-gray-200 dark:border-gray-700 flex justify-end gap-2">
              ...
              <Button onClick={handleSaveVip} disabled={actionLoading}>{actionLoading ? '保存中...' : '保存'}</Button>
            </div>
          </div>
        </div>
      )}
